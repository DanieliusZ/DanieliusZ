            ...
            <v-card style="width:100%">
                <v-list >
                    <v-list-tile v-for="(item, index) in workersInfo" :key="item.id" >
                        <v-list-tile-content >
                            <v-btn @click="selectedWorkerId=index" outline active-class="pactive" :class="{ pactive: selectedWorkerId===index }">
                            {{item.name}}                                
                            </v-btn>
                        </v-list-tile-content>
                    </v-list-tile>
...
export default {
    ...
    methods:{
        onMonthSelection(){
            ...
            let paymentInfo=(this.workersInfo[this.selectedWorkerId][this.year].payments)
            if(paymentInfo){
